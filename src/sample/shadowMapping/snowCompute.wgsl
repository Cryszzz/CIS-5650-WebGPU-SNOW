...
@group(0) @binding(2) var<uniform> weather_data_in: array<WeatherData, GRID_SIZE * GRID_SIZE>;
...
@group(0) @binding(5) var texture_out: texture_storage_2d<rgba32float, write>;
...
fn interpolate_swe(swe: f32, curvature: f32, inclination: f32) -> f32 {
  // let f = ((inclination < 15) ? 0.0 : (inclination / 60));
  let f = select(inclination / 60, 0.0, inclination < 15);
  let a3 = 50.0;
...
fn compute_main(@builtin(global_invocation_id) cell: vec3u)
{
  ...
  var cell_in = terrain_in[cell_index];
  var cell_out = terrain_out[cell_index];
  let weather_data_cell = weather_data_in[cell_index];
  let workgroup_size = vec2u(WORKGROUP_SIZE, WORKGROUP_SIZE);
  // let global_invocation_id_xy = global_invocation_id.xy;
  let tex_coord: vec2u = cell.xy * workgroup_size.xy; 
  ...
  if (weather_data_cell.precipitation > 0.0)
  {
    ...
    {
      cell_out.snow_albedo = cell_in.snow_albedo; //unsure if needed
      cell_out.snow_water_equivalent = cell_in.snow_water_equivalent 
  ...
  if (cell_out.snow_water_equivalent > 0.0)
  {
    ...
    if (weather_data_cell.temperature > TEMP_MELT)
    {
      ...
      cell_out.snow_water_equivalent = interpolate_swe(cell_in.snow_water_equivalent, cell_in.curvature, cell_in.inclination);
    }
    else
    {
      cell_out.snow_water_equivalent = cell_in.snow_water_equivalent;
